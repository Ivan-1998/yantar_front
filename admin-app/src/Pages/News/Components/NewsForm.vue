  ...
  >
    ...
    <div class="col-33p mb-30">
      <div class="form-group form-group-sm">
        ...
        <file-upload v-if="!photos.length && !news.photo"
                      v-model="photos"
...
export default {
  ...
  mounted() {
    const newsPhoto = this.news.photo;
    if (newsPhoto) {
      this.photos.push({name: newsPhoto});
    }
  },
  data() {
    return {
  ...
  methods: {
    ...
    validatePhoto() {
      if (!this.photos.length) {
        this.showToast({ text: 'Загрузите фотографию для новости', status: 'error' });
        return false;
      }
      return true;
    },
